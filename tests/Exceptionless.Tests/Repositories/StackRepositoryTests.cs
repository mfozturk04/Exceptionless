...
namespace Exceptionless.Tests.Repositories {
    public sealed class StackRepositoryTests : IntegrationTestsBase {
        ...
        [Fact]
        public async Task CanGetByApplicatonJoin() {
            ...
            var result = await _repository.GetIdsByFilterAsync(appFilter, "status:open OR status:regressed", DateTime.UtcNow.AddDays(-5), DateTime.UtcNow, o => o.PageLimit(10000));
            var stackIds = result.Hits.Select(h => h.Id).ToArray();
