...
namespace Exceptionless.Core.Repositories {
    public interface IStackRepository : IRepositoryOwnedByOrganizationAndProject<Stack> {
        ...
        Task<FindResults<Stack>> GetByFilterAsync(AppFilter systemFilter, string userFilter, string sort, string field, DateTime utcStart, DateTime utcEnd, CommandOptionsDescriptor<Stack> options = null);
        Task<FindResults<Stack>> GetIdsByFilterAsync(AppFilter systemFilter, string userFilter, DateTime utcStart, DateTime utcEnd, CommandOptionsDescriptor<Stack> options = null);
